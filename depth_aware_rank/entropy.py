...
from configs import cfg, cfg_from_file
...
palette = [128, 64, 128, 244, 35, 232, 70, 70, 70, 102, 102, 156, 190, 153, 153, 153, 153, 153, 250, 170, 30,
           220, 220, 0, 107, 142, 35, 70, 130, 180, 220, 20, 60, 255, 0, 0, 0, 0, 142,0, 60, 100, 0, 0, 230,
           119, 11, 32]
# palette = [128, 64, 128, 244, 35, 232, 70, 70, 70, 102, 102, 156, 190, 153, 153, 153, 153, 153, 250, 170, 30,
#            220, 220, 0, 107, 142, 35, 152, 251, 152, 70, 130, 180, 220, 20, 60, 255, 0, 0, 0, 0, 142, 0, 0, 70,
#            0, 60, 100, 0, 80, 100, 0, 0, 230, 119, 11, 32]
...
def colorize_save(output_pt_tensor, name):
    ...
    # mask_Img.save('./color_masks/%s.png' % (name.split('.')[0]))
...
def find_rare_class(output_pt_tensor):
    ...
    #mask_np_tensor = np.reshape(mask_np_tensor, 320*640)
...
#     state_dict ={k:v for k,v in saved_state_dict.items() if k in model_dict.keys()}
...
def main(args):
    ...
    # model_gen = get_deeplab_v2(num_classes=19, multi_level=True)
    model_gen = get_deeplab_v2_depth(num_classes=cfg.NUM_CLASSES,multi_level=False)

    restore_from = '/home/ailab/ailab/LJW/our_code/pretrained/model_13000_4_46.09.pth'

    print("Loading the generator:", restore_from)

    # model_gen.load_state_dict(torch.load(restore_from))
    # model_gen.eval()
    # model_gen.cuda(device)
    load_checkpoint_for_evaluation(model_gen, restore_from, device) # ISL
    ...
    for index in tqdm(range(len(target_loader))):
        ...
        with torch.no_grad():
            _, pred_trg_main, depth = model_gen(image.cuda(device))
            # _, pred_trg_main = model_gen(image.cuda(device))
            pred_trg_main = interp_target(pred_trg_main)
            depth = interp_target(depth)
            if args.normalize == True:
                normalizor = (11-len(find_rare_class(pred_trg_main))) / 11.0 + 0.5
            else:
                normalizor = 1
            pred_trg_entropy = prob_2_entropy(F.softmax(pred_trg_main))
            depth_aware = pred_trg_entropy*depth
            entropy_list.append((name[0],depth_aware.mean().item() * normalizor))
