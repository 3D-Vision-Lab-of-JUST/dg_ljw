...
if __name__ == "__main__":
    img_path = '/home/ailab/ailab/LJW/DADA/output/s2m_dada_ori_2000'
    dst_path = '/home/ailab/ailab/LJW/DADA/visualization/dada_s2m_2000_vis'
    ...
    for i in tqdm(range(len(list))):
        item = list[i]
        image_path = os.path.join(img_path, item)
        d_path = os.path.join(dst_path, item)
        if not os.path.exists(dst_path):
            os.mkdir(dst_path)
        #img = cv.imread(image_path)
        img = Image.open(image_path)
        #print("img_new:", type(img))

        img_new = colorize_mask(7,np.array(img))
        #print("img_new:",type(img_new))
        #dst = Image.fromarray(img_new)
        #dst = np.asarray(img_new)
        #dst = np.array(img_new)
        img_new.save(d_path)